...
export const Route = createFileRoute("/foundry")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "The Foundry — The Prodigy's Journey" },
      {
...
const journey = [
  { year: "2012", title: "First Spark", body: "Wins regional science fair with a hand-built mechanical prototype, age 14." },
  { year: "2015", title: "First Machine",  body: "Designs and builds his first functional industrial machine from scrap steel." },
  { year: "2018", title: "National Recognition", body: "Multiple national innovation awards. Featured in Ethiopian press as a young inventor of note." },
  { year: "2020", title: "ETH HD Metal Work", body: "Founds the company. Establishes the first dedicated workshop in Addis Ababa." },
  { year: "2023", title: "Industrial Floor", body: "Workshop expands to a full industrial floor — CNC, fabrication, electronics lab." },
  { year: "2026", title: "Operational at Scale", body: "12 active machines. Open for commissions, prototyping and venture partnerships." },
];
  ...
  return (
    <>
      <PageHero
        ...
        title={<>The Prodigy's <span className="text-electric">Journey.</span></>}
      ...
      <section className="relative mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
        <div className="grid gap-12 md:grid-cols-12">
          <div className="md:col-span-4 md:sticky md:top-32 md:self-start">
            <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-electric">· Timeline</div>
            <h2 className="mt-3 font-display text-3xl font-medium tracking-tight sm:text-4xl">
              From sketch <br /> to industrial floor.
            </h2>
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
              Each milestone added a layer of capability — until the workshop
              could build the machines that build other machines.
            </p>
          </div>

          <ol className="md:col-span-8 relative">
            <span className="absolute left-[1.125rem] top-2 bottom-2 w-px bg-hairline" aria-hidden />
            {journey.map((step, i) => (
              <li key={step.year} className="relative grid grid-cols-[2.5rem_1fr] gap-6 pb-12 last:pb-0">
                ...
                <div className="rise" style={{ animationDelay: `${i * 80}ms` }}>
                  <div className="font-mono text-xs text-electric tracking-[0.2em]">{step.year}</div>
                  <h3 className="mt-1 font-display text-2xl font-medium tracking-tight">{step.title}</h3>
      ...
      <section className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <figure className="relative overflow-hidden rounded-sm hairline-top hairline-bottom">
          <img src={workshopImg} alt="ETH HD Metal Work workshop interior, Addis Ababa" loading="lazy" width={1600} height={900} className="aspect-[16/9] w-full object-cover" />
      ...
      <section className="relative mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between gap-4 hairline-bottom pb-6">
          <div>
            <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-electric">· 02 / 02</div>
            <h2 className="mt-3 font-display text-3xl font-medium tracking-tight sm:text-4xl">
              Machine inventory.
            </h2>
          </div>
          ...
          {inventory.map((m) => (
            ...
            >
              ...
              <div className="text-right">
                ...
                >
                  <span className={`h-1.5 w-1.5 rounded-full ${m.status === "Operational" ? "bg-electric pulse-dot" : "bg-[oklch(0.78_0.16_55)]"}`} />
                  {m.status}
                </span>
              </div>
            </li>
