...
export const Route = createFileRoute("/investors")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Investor Command — ETH HD Metal Work" },
      {
      ...
      { name: "twitter:image", content: workshopImg },
    ],
  }),
...
const roadmap = [
  { phase: "Phase 01", year: "2026", title: "Foundry at Capacity",   body: "12 active machines, all service lines online, B2B commission flow." },
  { phase: "Phase 02", year: "2027", title: "Series Production",      body: "Spin out 2 flagship machines into repeatable industrial product lines." },
  { phase: "Phase 03", year: "2028", title: "Regional Distribution",  body: "Sales & service network into East Africa. Trained installer corps." },
  { phase: "Phase 04", year: "2030", title: "Pan-African Industrial", body: "Co-manufacturing partnerships across the continent." },
];

const tiers = [
  { name: "Sponsor a Prototype", commitment: "From $5K", desc: "Underwrite a single experimental machine from sketch to working unit.", intent: "sponsor" },
  { name: "Fund a Machine",      commitment: "From $50K", desc: "Bring a flagship machine to market — name, recognition and production rights.", intent: "sponsor" },
  { name: "Strategic Partner",   commitment: "Bespoke",   desc: "Equity, board seat and a co-authored continental roadmap.", intent: "invest" },
];
  ...
  return (
    <>
      <PageHero
        ...
        title={<>The African industrial <span className="text-electric">revolution</span> starts here.</>}
      ...
      <section className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="grid gap-10 md:grid-cols-12 md:items-end">
          <div className="md:col-span-5">
            <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-electric">· 06 / 01</div>
            <h2 className="mt-3 font-display text-3xl font-medium tracking-tight sm:text-4xl">
              Capacity trajectory.
            </h2>
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
              Indexed workshop output, 2021 → 2026. Each bar represents a year
              of compounding capability: more machines, more skilled hands,
              shorter cycle times.
            </p>
          </div>
                  ...
                  <div key={g.year} className="flex flex-1 flex-col items-center gap-2">
                    <div className="relative w-full">
                      ...
                      <div
                        ...
                        style={{ height: `${g.value * 1.5}px`, animationDelay: `${growth.indexOf(g) * 80}ms` }}
                      />
                    </div>
                    <div className="font-mono text-[10px] tracking-[0.15em] text-muted-foreground">{g.year}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="relative mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="hairline-top pt-12">
          <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-electric">· 06 / 02</div>
          <h2 className="mt-3 font-display text-3xl font-medium tracking-tight sm:text-4xl">
            ...
            {roadmap.map((r, i) => (
              <li key={r.phase} className="flex-1 bg-surface/60 p-6">
                <div className="flex items-center justify-between">
                  <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-electric">{r.phase}</div>
                  <div className="font-mono text-[10px] text-muted-foreground">{r.year}</div>
                </div>
                <div className="relative mt-4 mb-4 flex items-center">
                  <span className={`block h-3 w-3 rounded-full ${i === 0 ? "bg-electric pulse-dot" : "border border-electric/60 bg-background"}`} />
                </div>
      ...
      <section className="relative mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="hairline-top pt-12">
          <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-electric">· 06 / 03</div>
          <h2 className="mt-3 font-display text-3xl font-medium tracking-tight sm:text-4xl">
            Vision Support gateway.
          </h2>
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-muted-foreground">
            Interest capture only — no payment is taken on this site. Pick a
            tier; we'll reach out within 48 hours with a tailored brief.
          </p>
        </div>

        <div className="mt-10 grid gap-px bg-hairline/40 md:grid-cols-3">
          {tiers.map((t, i) => (
            <article key={t.name} className={`relative bg-surface/60 p-8 ${i === 1 ? "md:scale-[1.02]" : ""}`}>
              {i === 1 && (
              ...
              <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-electric">· Tier 0{i + 1}</div>
              ...
              <Link
                to="/contact"
                search={{ intent: t.intent, tier: t.name }}
