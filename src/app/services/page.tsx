...
const pillars = [
  {
    ...
    blurb:
      "From requirement brief to operational machine — engineered, fabricated and commissioned in-house.",
    capabilities: [
  ...
  },
  {
    ...
    blurb:
      "Production-grade metalwork: cutting, bending, welding, finishing — runs from a single unit to a full batch.",
    capabilities: [
      "CNC plasma cutting",
      "Press-brake bending",
      "MIG / TIG welding",
  ...
  },
  {
    ...
    blurb:
      "Rapid prototyping for hardware founders and R&D teams — from concept sketch to functional pre-production unit.",
    capabilities: [
      "3D printing (FDM & SLA)",
...
const process = [
  {
    step: "01",
    name: "Brief",
    body: "We meet, scope the build, and translate intent into a constrained problem statement.",
  },
  {
    step: "02",
    name: "Design",
    body: "CAD, simulation, and a costed bill of materials. Approved before a single cut.",
  },
  {
    step: "03",
    name: "Build",
    body: "Fabrication on the workshop floor. Daily progress on the Innovation Pulse.",
  },
  ...
  return (
    <>
      <PageHero
        ...
        title={
          <>
            The foundry, <span className="text-electric">on call.</span>
          </>
        }
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8 sm:py-24">
        <div className="grid gap-px bg-hairline/40 lg:grid-cols-3">
          {pillars.map((p) => (
            <article
              key={p.num}
              className="group relative bg-surface/60 p-8 transition-colors ease-mech hover:bg-surface-elevated sm:p-10"
            >
              ...
              <Link
                href={`/contact?intent=${p.intent}&service=${encodeURIComponent(p.title)}`}
