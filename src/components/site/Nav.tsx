  ...
  return (
    <header
      className={`fixed inset-x-0 top-0 z-50 transition-colors ease-mech duration-500 ${
        scrolled ? "bg-background/80 backdrop-blur-md hairline-bottom" : "bg-transparent"
      }`}
    >
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span className="relative block h-3 w-4">
              <span
                className={`absolute inset-x-0 top-0 h-px bg-foreground transition-transform ease-mech ${open ? "translate-y-1.5 rotate-45" : ""}`}
              />
              <span
                className={`absolute inset-x-0 top-1.5 h-px bg-foreground transition-opacity ${open ? "opacity-0" : ""}`}
              />
              <span
                className={`absolute inset-x-0 top-3 h-px bg-foreground transition-transform ease-mech ${open ? "-translate-y-1.5 -rotate-45" : ""}`}
              />
            </span>
          </button>
        </div>
